            ...
            ZStack {
                ...
                List {
                    Section("Cuenta") {
                        ...
                        LabeledContent("Fuente", value: catalog.sourceLabel)
                    }
                    Section("Datos") {
                        Text("Auth: POST /api/auth/login")
                        Text("Catálogo: GET /api/catalog (una vez → disco)")
                        Text("Streams: directo desde el iPhone al CDN")
                        Text("LiveContainer · SEÑAL 1.8.5")
                    }
                    Section("Lista") {
                        Button {
                            ...
                            if catalog.isLoading {
                                ...
                                Text("Actualizar lista desde servidor")
                            }
                        }
                        .disabled(catalog.isLoading || session.token == nil)
                    }
                    Section {
