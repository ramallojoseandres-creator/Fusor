...
enum RawHTTP {
    final class Session {
        ...
        private static func extractBody(from data: Data) throws -> Data {
            ...
            // Auth / API errors still include JSON body
    ...
    static func exchange(host: String, port: UInt16, request: Data) async throws -> Data {
    ...
    static func post(host: String, port: UInt16, request: Data) async throws -> Data {
        try await exchange(host: host, port: port, request: request)
    }
}
